...
<template>
  ...
  <el-input v-model="searchInfo.task_no" placeholder="搜索条件" />
</el-form-item>
  ...
  <el-input v-model.number="searchInfo.num" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="任务编号" prop="task_no" width="120" />

            <el-table-column align="left" label="数量" prop="num" width="120" />

            <el-table-column align="left" label="重量" prop="weight" width="120" />

            <el-table-column align="left" label="任务类型" prop="test_task_status" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.test_task_status,test_task_statusOptions) }}
    </template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button v-auth="btnAuth['新增']"  type="primary" link icon="edit" class="table-button" @click="updateBTaskFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getBTaskList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBTaskByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBTaskFunc = async(row) => {
    const res = await findBTask({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBTaskFunc = async (row) => {
    const res = await deleteBTask({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
